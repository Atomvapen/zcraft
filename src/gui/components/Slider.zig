...
pub const Slider = struct {
    ...
    pub fn init(pos: rl.Rectangle, minValue: f32, maxValue: f32, initialValue: *f32, thumbWidth: f32) Slider {
        return .{
            .pos = pos,
            .minValue = minValue,
            .maxValue = maxValue,
    ...
    pub fn draw(self: *Slider) void {
        ...
        rl.drawTexturePro(gui.buttonTexture, srcRect, self.pos, rl.Vector2{ .x = 0, .y = 0 }, 0, rl.Color.white);
        ...
        rl.drawTexturePro(if (self.isHovered()) gui.buttonHoveredTexture else gui.buttonTexture, srcRect, thumb, rl.Vector2{ .x = 0, .y = 0 }, 0, rl.Color.white);
